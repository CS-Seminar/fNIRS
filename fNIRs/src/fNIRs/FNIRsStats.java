...
public class FNIRsStats {
	...
	private static PrintWriter makePWriter(File file) {
		...
		try {
			...
			localError("unknown problem creating PrintWriter: "
					+ ex.getMessage());
		}
	...
	public static void writeANOVAs(File outFile, GroupedChannels data,
		...
		for (GroupedChannels.TaggedDataSequence tds : allTDSs) {
			ostream.printf(idFieldFormat + separator, tds.getGroupName() + " c"
					+ tds.getCondition());
		}
		...
		for (GroupedChannels.TaggedDataSequence first : allTDSs) {
			...
			ostream.printf(idFieldFormat + separator, first.getGroupName()
			...
			for (GroupedChannels.TaggedDataSequence second : allTDSs) {
				...
				if (result != result) { // if result is NaN
					System.out.println("NaN on " + first.getGroupName() + " c"
							+ first.getCondition() + " and "
	...
	private static int getIdFieldWidth(List<String> groupNames,
			List<Integer> conditions, int precision) {
	...
	public static void outputANOVAs(GroupedChannels data,
		...
		// then, output all tds identifiers:
		for (GroupedChannels.TaggedDataSequence tds : allTDSs) {
			System.out.printf(idFieldFormat + separator, tds.getGroupName()
					+ " c" + tds.getCondition());
		...
		for (GroupedChannels.TaggedDataSequence first : allTDSs) {
			// output tds identifier in first column:
			System.out.printf(idFieldFormat + separator, first.getGroupName()
					+ " c" + first.getCondition());
			// create Collection to send to the ANOVA object:
			LinkedList<double[]> dataSets = new LinkedList<double[]>();
			// convert first's data sequence to an array, then add it to
	...
	public static void oldWriteANOVAs(File outFile, GroupedChannels data,
		...
		// then, output all tds identifiers:
		for (GroupedChannels.TaggedDataSequence tds : allTDSs) {
			ostream.printf(idFieldFormat + "  ", tds.getGroupName() + " c"
					+ tds.getCondition());
		...
		for (GroupedChannels.TaggedDataSequence first : allTDSs) {
			// output tds identifier in first column:
			ostream.printf(idFieldFormat + "  ", first.getGroupName() + " c"
					+ first.getCondition());
			// create Collection to send to the ANOVA object:
			LinkedList<double[]> dataSets = new LinkedList<double[]>();
			// convert first's data sequence to an array, then add it to
	 ...
	 */
	private static <T> ArrayList<T> combineArrayLists(
			ArrayList<ArrayList<T>> lst_of_lsts) {
	...
	public static class GroupedChannels { // BETTER NAME PROBABLY
		...
		public void combineGroups(Group other) {
			...
			for (int i = 0; i < combinedSize; i++) {
				double ourSum = ours.getData().get(i) * ours.getNumChannels()
						* ours.NumSubjects;
				double otherSum = other.getData().get(i)
		 ...
		 */
		public ArrayList<GroupedChannels.TaggedDataSequence> getAllSelectedTDSs(
				List<String> groupNames, List<Integer> conditions) {
			return combineArrayLists(selectData(groupNames, conditions));
		}
		...
		public ArrayList<ArrayList<TaggedDataSequence>> selectData(
				List<String> groupNames, List<Integer> conditions) {
			...
			for (String groupName : groupNames) {
				...
				for (Integer condition : conditions) {
					...
					temp.add(new TaggedDataSequence(group, condition, group
							.getData(condition)));
		...
		class TaggedDataSequence {
			TaggedDataSequence(Group sourceGroup, int condition,
				...
				while (true) {
					...
					if (i == -1) { // -1 returned if not found
						...
						if (!avgSequence.isEmpty())
							...
							for (int j = 0; j < avgSequence.size(); j++)
								avgSequence
										.set(j,
												(avgSequence.get(j) / (double) numOccurrences));
					...
					do {
						...
						if (numOccurrences == 0) {
							...
							avgSequence
									.set(numRowsFound, (avgSequence
											.get(numRowsFound) + DataSequence
											.get(i)));
						}
						numRowsFound++;
	...
	private static void chunkTest(String data, int numChunks) {
		File groupFile = new File(
				"c:/Users/nkolesar/Desktop/CS Seminar/fNIRs/sub19/ChunkingTests/testGroups");
		File dataFile = new File(
				"C:/Users/nkolesar/Desktop/CS Seminar/fNIRs/sub19/ChunkingTests/"
						+ data);
	...
	private static void runSubjectTests() {
		...
		// test files:
		File groupFile = new File(
				"c:/Users/nkolesar/Desktop/CS Seminar/fNIRs/sub19/SubjectTests/groups");
		File data1 = new File(
				"C:/Users/nkolesar/Desktop/CS Seminar/fNIRs/sub19/SubjectTests/testData1");
		File data2 = new File(
				"C:/Users/nkolesar/Desktop/CS Seminar/fNIRs/sub19/SubjectTests/testData2");
		File data3 = new File(
				"C:/Users/nkolesar/Desktop/CS Seminar/fNIRs/sub19/SubjectTests/testData3");
	...
	private static void subjectTestsHelper(File[] dataFileAry, File groupFile) {
		...
		printList(processAllSubjectData(dataFileLst, groupFile).getGroup("foo")
				.getData(1));
	}
	...
	private static void runANOVATests() {
		...
		File groupFile = new File(
				"c:/Users/nkolesar/Desktop/CS Seminar/fNIRs/sub19/ANOVA Testing/legitGroups");
		File hbFile = new File(
				"C:/Users/nkolesar/Desktop/CS Seminar/fNIRs/sub19/ANOVA Testing/legitHb");
		int precision = 7;
		int numChunks = 10;
		...
		File outFile = new File(
				"C:/Users/nkolesar/Desktop/CS Seminar/fNIRs/sub19/ANOVA Testing/outputFile.csv");
		writeANOVAs(outFile, groups1, groups1.getGroupNames(),
				groups1.getConditions(),
	...
	private static void runANOVATestsHelper(GroupedChannels groups,
			String groupName, int condition) {
	...
	private static void runChannelsTests() {
		...
		File hbFile = new File(
				"C:/Users/nkolesar/Desktop/CS Seminar/fNIRs/sub19/channelWarningsTesting/legitHb");
	...
	public static void main(String[] args) {
		...
		File legitGroupsFile = new File(
				"c:/Users/nkolesar/Desktop/CS Seminar/fNIRs/sub19/legitGroups");
		File legitHbFile = new File(
				"C:/Users/nkolesar/Desktop/CS Seminar/fNIRs/sub19/legitHb");
		int precision = 7;
		int numChunks = 10;
